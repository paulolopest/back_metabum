...
describe('Insert Product test', () => {
	...
	test('Return when src is missing', async () => {
		expect.assertions;
		try {
			const result = await productBusinessMock.insertProduct(
				'ps5',
			...
			expect(error.message).toEqual('Enter a link for src');
		}
	});

	test('Return when src link is wrong', async () => {
		expect.assertions;
		try {
			const result = await productBusinessMock.insertProduct(
				'ps5',
...
describe('Edit Product Price test', () => {
	test('Return when authorization is missing', async () => {
		expect.assertions;
		try {
			const result = await productBusinessMock.editPrice(
				'',
				5000,
				'mocked_productId'
			);
...
describe('Delete Product test', () => {
	test('Return when authorization is missing', async () => {
		expect.assertions;
		try {
			const result = await productBusinessMock.deleteProduct(
				'',
				'product_id'
			);
